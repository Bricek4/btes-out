...
import { ArrowUpRight, GitBranch, PackageOpen } from '../lib/icons'
import type { Project } from '../types'

defineProps<{ project: Project; tone: 'violet' | 'cyan' | 'amber' | 'rose' }>()
...
<template>
  <article class="project-card" role="button" tabindex="0" :aria-label="`打开项目 ${project.name}`" @click="emit('open', project.projectId)" @keydown.enter="emit('open', project.projectId)" @keydown.space.prevent="emit('open', project.projectId)">
    <div class="project-card__head"><div class="project-card__glyph" :class="`project-card__glyph--${tone}`"><PackageOpen :size="19" :stroke-width="2" /></div><span v-if="project.shared" class="visibility-tag visibility-tag--public">已共享</span></div>
    <h3>{{ project.name }}</h3>
