...
import { ArrowUpRight, GitBranch, MoreHorizontal, PackageOpen } from 'lucide-vue-next'
...
<template>
  <article class="project-card" @click="emit('open', project.projectId)">
    <div class="project-card__head"><div class="project-card__glyph" :style="{ '--accent': accent }"><PackageOpen :size="19" /></div><button class="icon-button" type="button" aria-label="更多项目操作" @click.stop><MoreHorizontal :size="18" /></button></div>
    <h3>{{ project.name }}</h3>
