...
public enum InternalWorkerScope { AGENT, BROWSER, EITHER;
  public static InternalWorkerScope forPath(String path, String artifactKind) {
    if (path.matches("/internal/worker-context/agent/[0-9a-fA-F-]+") || path.matches("/internal/tasks/[0-9a-fA-F-]+/(provider-credential|events)")) return AGENT;
    ...
    if (path.matches("/internal/tasks/[0-9a-fA-F-]+/artifacts/presign") || path.matches("/internal/tasks/[0-9a-fA-F-]+/artifacts/[0-9a-fA-F-]+/complete")) {
      ...
      return "SCREENSHOT".equals(artifactKind) ? BROWSER : Set.of("DOC", "HTML", "MANIFEST").contains(artifactKind) ? AGENT : reject();
    }
    return reject();
  }
