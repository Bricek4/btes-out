...
public class ObjectStoreService {
  ...
  public Head head(String key){var h=s3.headObject(HeadObjectRequest.builder().bucket(bucket).key(key).build());return new Head(h.contentLength(),h.checksumSHA256());}
  public byte[] get(String key,long max){try(var in=s3.getObject(GetObjectRequest.builder().bucket(bucket).key(key).build())){var bytes=in.readAllBytes();if(bytes.length>max)throw new IllegalArgumentException("object exceeds read limit");return bytes;}catch(java.io.IOException e){throw new IllegalStateException("object read failed",e);}}
